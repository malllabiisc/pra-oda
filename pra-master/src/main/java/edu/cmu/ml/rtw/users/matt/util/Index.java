...
/**
 * A mapping from some object to integers, for any application where such a
 * mapping is useful (generally because working with integers is much faster and
 * less memory-intensive than working with objects).
 */
public class Index<T> {
	...
	public int getIndex(T key) {
		if (key == null) {
			throw new RuntimeException(
					"A null key was passed to the dictionary!");
		}
		...
		if (i == null) {
			...
			if (i == null) {
				if (verbose) {
					System.out.println("Key added to index at position "
							+ new_i + ": " + key + "; next index is "
							+ nextIndex.get());
				}
	...
	public void writeToWriter(FileWriter writer) throws IOException {
		if (nextIndex.get() > 20000000) {
			// This is approaching the size of something that can't fit in a
			// String object, so we
			// have to write it directly to disk, not use the printToString()
			// method.
